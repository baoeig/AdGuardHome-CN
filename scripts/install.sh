...
#
# See https://github.com/baoeig/AdGuardHome-CN/issues/2443.
fix_darwin() {
	if [ "$os" != 'darwin' ]; then
		return 0
	fi
...
rerun_with_root() {
	script_url='https://raw.githubusercontent.com/baoeig/AdGuardHome-CN/master/scripts/install.sh'
	readonly script_url
...
install_service() {
	...
	#
	# See https://github.com/baoeig/AdGuardHome-CN/issues/2542.
	if [ "$cpu" = 'armv7' ]; then
		cpu='armv5'
		reinstall='1'
